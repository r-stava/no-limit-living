...
router.post('/user', async (req, res) => {
    try {
        const newUser = new UserInput(req.body);
        const savedUser = await newUser.save();
        res.status(200).json(savedUser); // Return the saved user, including the _id
    } catch (error) {
...
// Route to handle prediction requests (without crime_data_year)
